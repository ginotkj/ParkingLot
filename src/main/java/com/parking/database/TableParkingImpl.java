...
public class TableParkingImpl implements TableParking{
	 ...
	 * @return
	 */
	private void createParkingLot(int lot){
	 ...
	 */
	private boolean add(Parking newParking) {
	...
	public void execute(String[] command) {
			...
			case registrationNumbersFoRCarsWithColour:
				...
				getRegistrationNumberByColour(params[0]);
				break;
			case slotNumbersForCarWithColour:
				if (!checkingParams(params, 1))
					return;
				
				getSlotNumber(params[0],"colour");
				break;
			case slotNumberForRegistrationNumber:
				if (!checkingParams(params, 1))
					return;
				
				getSlotNumber(params[0],"regno");
				break;
			case leave:
				if (!checkingParams(params, 1))
					return;
